...
def play(win, board):
    ...
    bWords = BoggleWords()
    # sets number of seconds until game ends to 3 minutes
    seconds = 180
    # sets up start time
    start = time.time()
    ...
    while not exitFlag:
        ...
        #current time is the active time
        currTime = time.time()
        #difference btwn currtime and start time is how many seconds have passed
        diff = currTime - start
        #because we are giving time in reverse order, timeleft is sec - diff
        timeLeft = int(seconds-diff)
        #display the time left
        board.setStringToUpperText(timeLeft)
        #continually updates seconds
        time.sleep(.1)

        # if the timer has run out, display text and reset everything
        if timeLeft <= 0:
            board.setStringToUpperText("   Time is up! Click to reset game.")
            point = win.getMouse()
            board.reset()
            bWords.reset()
            start = time.time()
        ...
        #once the player has clicked somewhere, start this portion of text
        if point:
